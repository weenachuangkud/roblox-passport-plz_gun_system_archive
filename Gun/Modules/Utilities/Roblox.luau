...
function Roblox.isPointInsidePart(point, part)
	...
	return math.abs(localPos.X) <= part.Size.X * 0.5
		and math.abs(localPos.Y) <= part.Size.Y * 0.5
		and math.abs(localPos.Z) <= part.Size.Z * 0.5
end
...
function Roblox.penetrateCast(ray, ignoreList)
	...
	while tries < 50 do
		...
		if
			hitPart
			and (not hitPart.CanCollide or CollectionService:HasTag(hitPart, "DroppedItemPart") or CollectionService:HasTag(
				hitPart,
				"Hidden"
			))
			and hitPart.Parent:FindFirstChildOfClass("Humanoid") == nil
		then
		...
		if not show then
			...
			coroutine.wrap(function()
				...
				local newTween = TweenService:Create(
					highlightInstance,
					TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.InOut, 0, true),
					{ ImageTransparency = 0 }
				)
...
function Roblox.getPointInFrontOnLines(referencePoint, forwardOffset, lines)
	...
	if closestPoint then
		...
		if closestLine == 1 and closestT == 0 then
			local beforeDist = (lines[1][1] - Roblox.getClosestPointOnLine(
				lines[1][1],
				lines[1][2],
				referencePoint,
				false
...
function Roblox.applySpread(unspreadDir, randomGenerator, minSpread, maxSpread)
	...
	local spreadTransform =
		CFrame.fromAxisAngle(Vector3.new(math.cos(spreadRotation), math.sin(spreadRotation), 0), spreadOffset)
